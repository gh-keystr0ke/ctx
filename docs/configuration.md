...
Required only to run `ctx ingest jira`. Jira Cloud only (Jira Server/Data Center isn't supported). Both `base_url` (e.g. `https://your-domain.atlassian.net`) and `project` (the legacy/default project key, e.g. `PSI`) remain required for configuration compatibility — unlike GitLab, Jira Cloud has no shared default host. `project` is informational: it does not filter candidate keys or supply an artifact's project. Credentials come only from the `CTX_JIRA_EMAIL` (the account the API token was issued under) and `CTX_JIRA_TOKEN` environment variables — never from a committed file:
...
`ctx ingest jira` never fetches an entire project — a Jira project routinely spans several unrelated services/repositories, so pulling all of it into each one would be mostly noise. Deterministically referenced keys from any project on the configured Jira Cloud site are eligible, and the response's `fields.project.key` supplies artifact project identity. The legacy default scans already-known artifact text. For repository isolation, prefer `ctx ingest jira --scope business-linked`: candidate keys then come only from current Git plus GitLab MRs selected by `ctx ingest gitlab --scope business-linked` and their comments; old Jira artifacts and unrelated GitLab issues cannot seed the set. `--related-depth 0` is the strict default, and a larger explicit value admits only that many Jira-reported `issuelinks`/`parent` hops. Run `ctx ingest git`, then strict GitLab ingestion, before strict Jira ingestion.

Routine Jira ingestion skips complete local issues and keys previously confirmed inaccessible or nonexistent. Those negative-cache entries do not expire automatically: use `ctx ingest jira --refresh` to revalidate every current candidate and replace the cache after a completely successful run. The same flag makes business-linked GitLab refetch every matching MR, or makes all-scope GitLab ignore its stored cursor once. Jira issues and business-linked GitLab MRs therefore remain at their first complete local version until refresh; GitLab all-scope stays update-aware through its normal cursor. Refresh also repairs a suspected partial ingest written by a pre-0.8.5 build. `--refresh` is rejected for local-only `git` and `code-comments` ingestion.
