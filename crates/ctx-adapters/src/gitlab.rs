...
use ctx_app::ports::{GitLabArtifactSource, PortError};
...
impl GitLabTransport for UreqTransport {
    fn get(&self, path: &str) -> Result<String, GitLabError> {
        ...
        let mut response = request.call().map_err(|error| GitLabError::Transport {
            path: path.to_owned(),
...
impl<T: GitLabTransport> GitLabClient<T> {
    ...
    pub fn fetch_issue_and_mr_artifacts(
        ...
        for issue in self.get_json::<Vec<RawIssue>>(&format!(
            ...
            let notes = self.get_json::<Vec<RawNote>>(&format!(
        ...
        for merge_request in self.get_json::<Vec<RawMergeRequest>>(&format!(
            ...
            let commits = self.get_json::<Vec<RawCommitRef>>(&format!(
...
mod tests {
    ...
    fn fetches_issues_and_merge_requests_with_their_comments_and_commits() {
        ...
        responses.insert(
            "/projects/billing%2Fsubscriptions/issues?per_page=100&order_by=iid&sort=asc"
                .to_owned(),
        ...
        responses.insert(
            "/projects/billing%2Fsubscriptions/issues/317/notes?per_page=100".to_owned(),
        ...
        responses.insert(
            "/projects/billing%2Fsubscriptions/merge_requests?per_page=100&order_by=iid&sort=asc"
                .to_owned(),
        ...
        responses.insert(
            "/projects/billing%2Fsubscriptions/merge_requests/842/notes?per_page=100".to_owned(),
            "[]".to_owned(),
        );
        responses.insert(
            "/projects/billing%2Fsubscriptions/merge_requests/842/commits?per_page=100".to_owned(),
