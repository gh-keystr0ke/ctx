...
A `goose` (<https://github.com/pressly/goose>) module is registered beside the language analyzers, dispatching on the `.sql` extension. It reads only a migration file's `-- +goose Up` section (`-- +goose Down` describes how a change is reverted, not what is currently declared) and recognizes common `CREATE TABLE`/`ALTER TABLE ... ADD COLUMN` forms with a dedicated deterministic DDL reader (`ctx-adapters::database::ddl_table_columns`) — not a SQL dialect parser and not Tree-sitter-based, so its edges carry the `goose_ddl_recognizer` producer rather than the `<language>_tree_sitter` convention every grammar-backed module uses. Table constraints (`PRIMARY KEY`, `FOREIGN KEY`, `UNIQUE`, `CHECK`, `CONSTRAINT`, `INDEX`) are skipped rather than misread as columns; a file with no goose annotations, or no recognizable DDL, yields zero symbols instead of an error or a guess.

Each migration file becomes one `SchemaMigration`-kind symbol (canonical path derived from its path, for example `migrations.20240101000000_create_subscriptions`) carrying zero or more declared tables and their columns. This reuses the existing per-file commit-bounded versioning, identity, and retirement machinery symbols already have — a migration is not a new kind of storage concept, just a new symbol kind and a new `DEFINES_SCHEMA` `FACT` edge kind alongside `READS_FROM`/`WRITES_TO`, pointing at the same `DbEntity` graph those statically-recognized SQL accesses use. A table declared only by a migration, never read or written by any indexed code, still becomes a `DbEntity` and appears in impact/review/Context Pack like any other data contract. Column names/types are carried as evidence on the edge's locator (`columns:name:type,...`) rather than a separate schema table, so `ctx explain` can show them without new query surface. `ctx` deliberately does not merge multiple migrations' declarations for the same table into one computed "current" schema (that would require a dialect-complete parser and an Alembic/goose-version-graph-aware resolver); each migration file's declaration remains its own fact with its own evidence, and a reader assembles the history from the graph the same way they would read the migration directory itself.

The Python module recognizes `SQLAlchemy` declarative models with the same `DEFINES_SCHEMA`/`SchemaMigration`-shaped fact (a class is the versioned unit instead of a migration file). Only a class that assigns a static string literal to `__tablename__` — the framework-mandated declarative-mapping marker — is treated as a model at all; declarative-base detection (resolving `class X(Base):`) is deliberately not attempted, since aliasing and inheritance make it unreliable without import resolution, while `__tablename__` is unambiguous. Within a recognized model, `name = Column(...)`/`name = mapped_column(...)` attribute assignments become columns, with the type read from the constructor's first positional argument (or the variable's type annotation, for the annotation-only `Mapped[int]` style, or `"unknown"` if neither is present) — not a SQLAlchemy type-system parser. A dynamic `__tablename__` (anything other than a plain string literal) makes the whole class unrecognized, matching the project's static-SQL-literal rule of never guessing from dynamic expressions.
