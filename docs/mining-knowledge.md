...
`ctx ingest <git|code-comments|gitlab|jira>` normalizes artifacts into their own store. One network response's artifacts and deterministic links commit atomically, so a failed link write cannot leave a root that suppresses retry. `--since <OID>` narrows `git` to commits after that point (branch names are always re-synced). The default `gitlab` mode stores a per-project sync cursor and ingests issues plus MRs. `ctx ingest gitlab --scope business-linked` deliberately does something narrower: it relists MR summaries, selects only those connected to current Git by source branch, commit SHA, or an explicit `!IID`, and fetches comments/commits only for selected MRs not already complete locally. It does not fetch GitLab issues or advance the default-mode cursor.

`jira` doesn't use a time cursor at all: since a Jira project can span many repositories, it never fetches "the whole project, or everything changed since last time." Referenced keys can belong to any project on the configured Jira Cloud site. In business-linked mode, every run derives ticket keys (`PSI-1122`) only from current Git, selected MRs, and their comments. Old Jira rows and unrelated GitLab issues cannot seed or perpetuate the set. `--related-depth 0` admits only directly referenced issues; increase it explicitly to follow a bounded number of Jira-reported `issuelinks`/`parent` hops.

Routine Jira runs skip complete local issues and a persistent negative cache of confirmed unavailable keys; business-linked GitLab likewise skips complete local MRs after summary matching. This deliberately trades automatic freshness for fewer provider calls. Use `ctx ingest jira --refresh` or `ctx ingest gitlab --refresh` to bypass those skips (and the GitLab all-scope cursor) and rebuild provider state. Jira negative entries are permanent until such a fully successful refresh replaces them; a failed refresh preserves the previous cache. Refresh also repairs suspected partial data from older, non-atomic ingestion versions.
